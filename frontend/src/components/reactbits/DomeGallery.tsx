...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    if (!root) return;
    /* Set segments vars immediately so CSS calcs work before ResizeObserver fires */
  ...
  useGesture({
    ...
    onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
      ...
      if (last) {
        ...
        let isTap = false;
        if (startPosRef.current) { const dx = evt.clientX - startPosRef.current.x, dy = evt.clientY - startPosRef.current.y; const TAP = pointerTypeRef.current === 'touch' ? 10 : 6; if (dx * dx + dy * dy <= TAP * TAP) isTap = true; }
        let [vMagX, vMagY] = velArr; const [dirX, dirY] = dirArr; let vx = vMagX * dirX, vy = vMagY * dirY;
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (!originalPos) {
        ...
        el.style.visibility = ''; (el.style as any).zIndex = 0; focusedElRef.current = null;
        rootRef.current?.removeAttribute('data-enlarging'); openingRef.current = false; return;
      }
      ...
      const oImg = overlay.querySelector('img'); if (oImg) { const img = oImg.cloneNode() as HTMLImageElement; img.style.cssText = 'width:100%;height:100%;object-fit:cover;'; anim.appendChild(img); }
      ...
      anim.addEventListener('transitionend', () => {
        ...
        if (refDiv) refDiv.remove(); parent.style.transition = 'none'; el.style.transition = 'none';
        parent.style.setProperty('--rot-y-delta', '0deg'); parent.style.setProperty('--rot-x-delta', '0deg');
        requestAnimationFrame(() => {
          el.style.visibility = ''; el.style.opacity = '0'; (el.style as any).zIndex = 0; focusedElRef.current = null; rootRef.current?.removeAttribute('data-enlarging');
          requestAnimationFrame(() => {
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const parent = el.parentElement as HTMLElement; focusedElRef.current = el; el.setAttribute('data-focused', 'true');
