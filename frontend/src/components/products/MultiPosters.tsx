...
function MultiCard({ p }: { p: typeof products[0] }) {
  ...
  return (
    <div style={{ cursor: "pointer" }}>
      <div style={{ position: "relative", aspectRatio: "1/1", overflow: "hidden", marginBottom: 12, background: "var(--c-bg-soft)", borderRadius: 6 }}
        ...
        <div style={{ position: "absolute", bottom: 10, left: 10, right: 10 }}>
          <ClickSpark sparkColor="#fff" sparkCount={8} sparkRadius={20}>
            <button onClick={handleQuickAdd}
              style={{ width: "100%", padding: "12px 0", background: "rgba(10,10,10,0.88)", backdropFilter: "blur(4px)", color: "#fff", fontFamily: FO, fontSize: 11, fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", border: "none", cursor: "pointer", borderRadius: 8 }}>
              Quick Add
            </button>
          </ClickSpark>
        </div>
      ...
      <Link href={`/product/${p.id}`} style={{ textDecoration: "none" }}>
        ...
        <div style={{ fontFamily: F, fontSize: 10, color: "#aaa", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.04em", textAlign: "center" }}>{p.sub}</div>
