      ...
      <Link href={`/product/${p.id}`} style={{ textDecoration: "none", display: "block" }}>
        <div
          ...
          onMouseEnter={e => {
            ...
            const btn = e.currentTarget.querySelector(".bs-qa") as HTMLElement;
        ...
        >
          ...
          <div className="bs-qa" style={{ position: "absolute", bottom: 0, left: 0, right: 0, opacity: 0, transform: "translateY(10px)", transition: "all 0.25s ease" }}>
            <ClickSpark sparkColor="#fff" sparkCount={8} sparkRadius={20}>
              <button onClick={handleQuickAdd} style={{ width: "100%", padding: "13px 0", background: "rgba(10,10,10,0.92)", backdropFilter: "blur(6px)", color: "#fff", fontFamily: FO, fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", border: "none", cursor: "pointer", borderRadius: 50 }}>
                Quick Add
              </button>
            </ClickSpark>
          </div>
        ...
        <div style={{ textAlign: "center", padding: "0 4px" }}>
          <div style={{ fontFamily: FO, fontSize: 12, fontWeight: 500, color: "var(--c-text)", textTransform: "uppercase", marginBottom: 4, lineHeight: 1.45, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const }}>{p.title}</div>
    ...
    <section style={{ padding: "64px 0", background: "var(--c-bg)" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
          <h2 style={{ fontFamily: FE, fontSize: "clamp(22px,3vw,40px)", fontWeight: 400, color: "var(--c-text)", textTransform: "uppercase", letterSpacing: "-0.03em", margin: 0, display: "flex", gap: "0.2em", alignItems: "center", flexWrap: "wrap" }}>
