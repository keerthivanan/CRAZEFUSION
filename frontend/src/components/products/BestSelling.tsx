    ...
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 10 }}>
      ...
      <ClickSpark sparkColor="#fff" sparkCount={8} sparkRadius={20}>
        <button onClick={handleQuickAdd}
          style={{ width: "100%", padding: "11px 0", background: "rgba(10,10,10,0.88)", backdropFilter: "blur(6px)", color: "#fff", fontFamily: FO, fontSize: 11, fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", border: "none", cursor: "pointer", borderRadius: 8 }}>
          Quick Add
        </button>
      </ClickSpark>
    </div>
    ...
    <SpotlightCard spotlightColor="rgba(160,160,160,0.1)" style={{ padding: 10, background: "var(--c-bg)", border: "1px solid var(--c-border)", borderRadius: 8 }}>
      <Link href={`/product/${p.id}`} style={{ textDecoration: "none", display: "block" }}>
        <TiltedCard
          ...
          borderRadius="4px" displayOverlayContent overlayContent={overlay}
        />
        <div style={{ padding: "12px 4px 4px", textAlign: "center" }}>
          <div style={{ fontFamily: FO, fontSize: 12, fontWeight: 400, color: "var(--c-text)", textTransform: "uppercase", marginBottom: 4, lineHeight: 1.3, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const }}>{p.title}</div>
          <div style={{ fontFamily: F, fontSize: 10, color: "#aaa", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.04em" }}>{p.sub}</div>
