...
const PACKS = [
  ...
  { id: "creator", name: "Creator", price: 7.99,  credits: 100, edits: 20, color: "linear-gradient(135deg,#1a1200,#2a1e00)", border: "#e8a000", accentColor: "#e8a000", tag: "Most Popular" },
    ...
    <>
      ...
      <div style={{ background: "#080808", minHeight: "100vh", paddingTop: 100, fontFamily: FO }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px 64px", textAlign: "center" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(232,160,0,0.1)", border: "1px solid rgba(232,160,0,0.25)", borderRadius: 50, padding: "6px 16px", marginBottom: 28 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#e8a000", display: "inline-block" }} />
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", color: "#e8a000" }}>AI Credits</span>
          </div>
          <h1 style={{ fontSize: "clamp(36px,5vw,64px)", fontWeight: 800, color: "#fff", margin: "0 0 16px", letterSpacing: "-0.03em", lineHeight: 1.1 }}>
            Power Your<br /><span style={{ color: "#e8a000" }}>Creativity</span>
          </h1>
          ...
          {/* Balance badge */}
          {balance !== null && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(232,160,0,0.06)", border: "1px solid rgba(232,160,0,0.15)", borderRadius: 50, padding: "8px 20px" }}>
              <span style={{ fontSize: 13, color: "#888" }}>Your balance:</span>
              <span style={{ fontSize: 18, fontWeight: 800, color: "#e8a000" }}>{balance} credits</span>
            </div>
          )}
        </div>
        ...
        {success && (
          <div style={{ maxWidth: 480, margin: "0 auto 32px", padding: "16px 24px", background: "rgba(22,163,74,0.08)", border: "1px solid rgba(22,163,74,0.2)", borderRadius: 14, textAlign: "center" }}>
            ...
            <Link href="/create" style={{ fontSize: 12, color: "#e8a000", textDecoration: "none", display: "block", marginTop: 8 }}>Go to AI Studio</Link>
          </div>
        )}
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px 80px", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 20 }}>
          {PACKS.map((p, i) => (
            <div key={p.name} style={{ background: p.color, border: `1px solid ${p.border}`, borderRadius: 20, padding: 32, position: "relative", overflow: "hidden", transition: "transform 0.2s" }}
              ...
              {p.tag && (
                <div style={{ position: "absolute", top: 16, right: 16, background: i === 1 ? "#e8a000" : "#6366f1", color: i === 1 ? "#000" : "#fff", fontSize: 9, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", padding: "4px 10px", borderRadius: 50 }}>
                  {p.tag}
                </div>
              )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {WHAT.map(w => (
              <div key={w.action} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", background: "rgba(255,255,255,0.02)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: "#e8a000", background: "rgba(232,160,0,0.08)", padding: "4px 12px", borderRadius: 20, whiteSpace: "nowrap", marginLeft: 16 }}>
                  {w.credits} credits
                </span>
              </div>
            ))}
        ...
        <div style={{ textAlign: "center", padding: "0 24px 100px" }}>
          <Link href="/create" style={{ display: "inline-block", padding: "15px 40px", background: "#e8a000", color: "#000", fontFamily: FO, fontSize: 12, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", textDecoration: "none", borderRadius: 50, boxShadow: "0 0 32px rgba(232,160,0,0.25)", transition: "all 0.2s" }}
            onMouseEnter={e => (e.currentTarget.style.boxShadow = "0 0 48px rgba(232,160,0,0.45)")}
            onMouseLeave={e => (e.currentTarget.style.boxShadow = "0 0 32px rgba(232,160,0,0.25)")}>
            Try The AI Studio
          </Link>
        </div>
