...
const INPUT_DIR  = path.join(__dirname, '..', 'keerthicollections', 'bar-posters');
const OUTPUT_DIR = path.join(__dirname, '..', 'mockups', 'bar-posters');
const CONCURRENCY = 6;
...
async function main() {
  ...
  console.log(`Generating ${total} bar-poster mockups → mockups/bar-posters/\n`);
  ...
  for (let i = 0; i < files.length; i += CONCURRENCY) {
    ...
    const results = await Promise.allSettled(batch.map(f => {
      const out = path.join(OUTPUT_DIR, f.replace(/\.(webp|jpeg|png)$/i, '.jpg'));
  ...
  console.log(`\nDone! ${done - failed} mockups → mockups/bar-posters/`);
}
